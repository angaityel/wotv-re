...
function StateDedicatedServerInit:on_enter()
	...
	GameSettingsDevelopment.game_server_created_callback()
	Managers.persistence:post_init()

	function World.create_particles()
		return 1
	end

	function World.destroy_particles()
		return
	end

	function World.stop_spawning_particles()
		return
	end

	function World.move_particles()
		return
	end

	function World.link_particles()
		return
	end

	function World.set_particles_variable()
		return
	end
end
...
function StateDedicatedServerInit:update(dt)
	...
	if lobby_manager.state == LobbyState.JOINED then
		...
		for _, map in ipairs(map_rotation) do
			...
			Managers.lobby:set_lobby_data("message_of_the_day", game_server_settings.message_of_the_day)

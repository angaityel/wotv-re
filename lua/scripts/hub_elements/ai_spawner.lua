...
function AISpawner:_spawn_unit()
	...
	local unit_name = DefaultUnits.standard.ai
...
function AISpawner:change_behaviour(behaviour_name)
	for unit, _ in pairs(self._spawned_units) do
		local ai_system = ScriptUnit.extension(unit, "ai_system")

		ai_system:brain():change_behaviour("main", behaviour_name)
	end
end

function AISpawner:set_ai_invulnerable()
	for unit, _ in pairs(self._spawned_units) do
		local damage = ScriptUnit.extension(unit, "damage_system")

		damage:set_invulnerable(true)
	end
end

function AISpawner:set_ai_vulnerable()
	for unit, _ in pairs(self._spawned_units) do
		local damage = ScriptUnit.extension(unit, "damage_system")

		damage:set_invulnerable(false)
	end
end

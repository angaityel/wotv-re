...
function ScriptInputSource:get(name)
	...
	if self.global_mapping then
		...
		input_desc = self.mapping_table[active_mapping] and self.mapping_table[active_mapping][name]
	else
		input_desc = self.mapping_table[name]
	end
...
function ScriptInputSource:_update_input(dt, t)
	...
	for i = #self._input_settings_copy, 1, -1 do
		...
		if self._active_time > config.start then
			...
			input_desc = self.mapping_table[active_mapping] and self.mapping_table[active_mapping][config.name]

			if self.global_mapping then
				-- block empty
			else
				input_desc = self.mapping_table[config.name]
			end

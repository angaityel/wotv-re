...
function DebugTextManager:_update_screen_text()
	for identifier, text_item in pairs(self._screen_text_list) do
		if text_item.time < self._time then
			Gui.destroy_text(self._gui, text_item.text_id)
			Gui.destroy_rect(self._gui, text_item.bgr_id)

			self._screen_text_list[identifier] = nil
		end
	end
end
...
function DebugTextManager:output_screen_text(text, text_size, time, color, unique_identifier, x, y)
	...
	if x and y then
		text_position = Vector3(x, y, 11)
	else
		text_position = Vector3(resolution.x / 2 - text_w / 2, resolution.y / 2 - text_h / 2, 11)
	end
	...
	unique_identifier = unique_identifier or "default"

	local text_item = self._screen_text_list[unique_identifier]

	if text_item then
		Gui.update_text(gui, text_item.text_id, text, font, text_size, material, text_position, Color(color.x, color.y, color.z))
		Gui.update_rect(gui, text_item.bgr_id, bgr_position, bgr_size, Color(120, 0, 0, 0))

		text_item.time = self._time + (time or self._screen_text_time)
	else
		local screen_text = {
		...
		self._screen_text_list[unique_identifier] = screen_text
	end
end

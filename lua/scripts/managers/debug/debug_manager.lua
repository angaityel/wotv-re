...
function DebugManager:update(dt)
	...
	local tests = self._tests

	for i = #tests, 1, -1 do
		local test_data = tests[i]
		local done = test_data.update_function(dt, test_data.data)
...
function DebugManager:register_test(data, update_function)
...
function DebugManager:clear_tests()
	table.clear(self._tests)
end

function DebugManager:unregister_test(data)
	for i = #tests, 1, -1 do
		local test_data = tests[i]
...
function DebugManager:reset_drawers()
	for _, drawer in pairs(self._drawers) do
		drawer:reset()
	end
end

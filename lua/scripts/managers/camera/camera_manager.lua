...
function CameraManager:_update_shadow_lights(dt, viewport)
	...
	if self._shadow_lights_active and viewport == self._shadow_lights_viewport and not table.is_empty(lights) then
		...
		for i = 1, math.min(self._shadow_lights_max_active, #lights) do
...
function CameraManager:shading_callback(world, shading_env)
	if self._world == world then
		...
		for name, settings in pairs(HUDSettings.outline_colors) do
...
function CameraManager:set_vignette(falloff_opacity, color, t)
	if falloff_opacity then
		self._vignette_falloff_opacity = Vector3Box(falloff_opacity)
		self._vignette_color = Vector3Box(color)
		self._vignette_t = t
	else
		self._vignette_falloff_opacity = nil
		self._vignette_color = nil
		self._vignette_t = nil
	end
end
...
function CameraManager:_smooth_camera_collision(camera_position, safe_position, smooth_radius, near_radius)
	...
	while true do
		...
		self._hits = PhysicsWorld.linear_sphere_sweep(physics_world, cast_from, cast_to, cast_radius, 1, "types", "both", "collision_filter", "camera_sweep")
...
function CameraManager:_update_camera(dt, viewport_name)
	...
	if self._sequence_event_settings.event then
		camera_data.position, camera_data.rotation = self:_apply_sequence_event(camera_data, t)
	end

	for settings, _ in pairs(self._shake_event_settings) do
		camera_data.rotation = self:_apply_shake_event(settings, camera_data, t)
	end
...
function CameraManager:_apply_sequence_event(current_data, t)
	...
	local position = self:_calculate_sequence_event_position(current_data, new_values)
	local rotation = self:_calculate_sequence_event_rotation(current_data, new_values)
...
function CameraManager:_apply_shake_event(settings, current_data, t)
	...
	local rotation = Quaternion.multiply(current_rot, total_offset)

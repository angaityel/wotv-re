...
function BackendManager:connect(ip_address, project_id, connection_type, port, interface, callback, timeout)
	...
	Managers.token:register_token(backend_token, callback, timeout)
end

function BackendManager:_timeout()
	return Managers.time:time("main") + GameSettingsDevelopment.backend_timeout or 60
end
...
function BackendManager:login(username, password, callback)
	...
	Managers.token:register_token(backend_token, callback, self:_timeout())
end

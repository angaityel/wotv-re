...
function HUDDeserting:init(world, player)
	...
	self._world = world
	self._player = player
	self._gui = World.create_screen_gui(world, "material", MenuSettings.font_group_materials.arial, "immediate")
...
function HUDDeserting:post_update(dt, t)
	...
	if announcement_time_param ~= self._announcement_time_param and self._deserting_timer > 0 then

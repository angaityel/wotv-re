...
function HUDXPCoins:init(world, player)
	...
	self._world = world
	self._player = player
	self._gui = World.create_screen_gui(world, "material", MenuSettings.font_group_materials.hell_shark, "immediate")
...
function HUDXPCoins:hud_manager_inactive_post_update(dt, t)
...
function HUDXPCoins:_update_queued_awards(dt, t)
	...
	if t >= award:start_time() then
		...
		local event_id = TimpaniWorld.trigger_event(timpani_world, "hud_exp")
	end
end
...
function HUDXPCoins:_update_showing_awards(dt, t)
	for index, award in ipairs(self._showing_awards) do
		...
		if award:update(dt, t, x, y - (index - 1) * layout_settings.font_size, layout_settings, self._gui) then

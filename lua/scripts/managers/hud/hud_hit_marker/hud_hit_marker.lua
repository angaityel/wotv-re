...
function HUDHitMarker:init(world, player)
	...
	Managers.state.event:register(self, "event_hit_marker_activated", "event_hit_marker_activated")
end
...
function HUDHitMarker:post_update(dt, t)
	local marker = self._hit_marker_container:element("marker")

	if (not self._active or not marker.config.active and not marker:active()) and not script_data.debug_hit_marker and not script_data.debug_team_hit_marker then
		return
	end

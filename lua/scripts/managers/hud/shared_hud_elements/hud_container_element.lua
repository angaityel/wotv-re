...
function HUDContainerElement:remove_element(id)
	if self._elements[id].destroy then
		self._elements[id]:destroy()
	end
	...
	for id, element in pairs(self._elements) do
		if element.is_table then
			for _, element_child in ipairs(element) do
				element_child:render(dt, t, gui, HUDHelper:layout_settings(element_child.config.layout_settings))
			end
		else

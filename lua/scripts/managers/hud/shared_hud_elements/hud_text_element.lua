...
function HUDTextElement:update_size(dt, t, gui, layout_settings)
	...
	if layout_settings.text_max_length then
		text = HUDHelper:truncate_text(text, layout_settings.text_max_length, "...", true)
	end
...
function HUDTextElement:render(dt, t, gui, layout_settings)
	local config = self.config
	local color_table = config.blackboard and config.blackboard.color and config.blackboard.color or config.text_color or layout_settings.text_color
	local color = Color(color_table[1], color_table[2], color_table[3], color_table[4])
	local shadow_color = config.shadow_color or layout_settings.shadow_color and Color(layout_settings.shadow_color[1], layout_settings.shadow_color[2], layout_settings.shadow_color[3], layout_settings.shadow_color[4])

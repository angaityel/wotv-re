...
function HUDTextureElement:_texture_atlas_settings(layout_settings)
	return layout_settings.texture_atlas_settings_func and layout_settings.texture_atlas_settings_func(self.config.blackboard) or layout_settings.texture_atlas_settings or self.config.texture_atlas_settings
end
...
function HUDTextureElement:_color(dt, t, gui, layout_settings, config)
	if layout_settings.color_func then
		return layout_settings.color_func(layout_settings, config)
	end
...
function HUDTextureElement:_texture(dt, t, gui, layout_settings, config)
	return config.texture or config.blackboard and config.blackboard.texture or layout_settings.texture
end
...
function HUDTextureElement:render(dt, t, gui, layout_settings)
	...
	if config.gradient_shader_value then
		local texture = layout_settings.texture_atlas or config.texture_atlas or self:_texture(dt, t, gui, layout_settings, config)
		local material = Gui.material(gui, texture)
	...
	if layout_settings.texture_atlas or config.texture_atlas then
		...
		if config.gradient_shader_value then
			...
			if material then
				Material.set_vector2(material, "gradient_uv_offset", uv00)
				Material.set_vector2(material, "gradient_uv_scale", Vector2(1 / (uv11.x - uv00.x), 1 / (uv11.y - uv00.y)))
			end
		end

		Gui.bitmap_uv(gui, layout_settings.texture_atlas or config.texture_atlas, uv00, uv11, position, size, color)
	else

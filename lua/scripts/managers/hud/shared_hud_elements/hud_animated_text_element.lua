...
function HUDAnimatedTextElement:x()
	return self._x
end

function HUDAnimatedTextElement:y()
	return self._y
end

function HUDAnimatedTextElement:z()
	return self._z
end

function HUDAnimatedTextElement:reset_timer()
	self._t = 0
end
...
function HUDAnimatedTextElement:render(dt, t, gui, layout_settings)
	...
	for i = #self._text, 1, -1 do
		...
		if config.drop_shadow then
			local alpha = self.alpha_multiplier or 1
			local drop_shadow_color = config.drop_shadow.color:unbox()
			local a, r, g, b = Quaternion.to_elements(drop_shadow_color)

			ScriptGUI.text(gui, text.text, font.font, font_size, font.material, Vector3(math.floor(x), math.floor(y + y_offset), self._z), Color(color[1] * alpha, color[2], color[3], color[4]), Color(alpha * a, r, g, b), config.drop_shadow.offset:unbox())
		else

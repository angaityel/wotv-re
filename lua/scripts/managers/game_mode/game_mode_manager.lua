...
function GameModeManager:init(world, game_mode_key, level_key, win_score, time_limit, server_game_mode_scale)
	...
	event_manager:register(self, "objective_unlocked_by_time", "event_objective_unlocked_by_time")
...
function GameModeManager:gm_event_end_conditions_met()
...
function GameModeManager:add_objective_boundary_area(...)
	if self._game_mode.add_objective_boundary_area then
		self._game_mode:add_objective_boundary_area(...)
	end
end

function GameModeManager:add_objective_spawn_point(objective_unit, side, spawn_point_name, spawn_direction)
	if self._game_mode.add_objective_spawn_point then
		self._game_mode:add_objective_spawn_point(objective_unit, side, spawn_point_name, spawn_direction)
	end
end

function GameModeManager:set_objective_point_order(points)
	if self._game_mode.set_objective_point_order then
		self._game_mode:set_objective_point_order(points)
	end
end
...
function GameModeManager:allow_flag_spawn()
	return GameModeSettings[self._game_mode_key].allow_flag_spawn
end

function GameModeManager:allow_ghost_talking()
	return self._gm_event_end_conditions_met or GameModeSettings[self._game_mode_key].allow_ghost_talking
end

function GameModeManager:allowed_spawn_modes(player)
	return self._game_mode:allowed_spawn_modes(player)
end
...
function GameModeManager:_set_flow_object_set_enabled(set, enable)
	if enable == set.enabled then
		return
	end
...
function GameModeManager:flow_cb_set_flow_object_set_enabled(set_name, enabled)
	...
	self:_set_flow_object_set_enabled(set, enabled)

	if Managers.lobby.server then
		Managers.state.network:send_rpc_clients("rpc_set_flow_object_set_enabled", set.key, enabled)
	end
end
...
function GameModeManager:register_object_sets(object_sets)
	...
	for set_name, set in pairs(object_sets) do
		...
		elseif set.type == "flow" then
			self:_set_flow_object_set_enabled(set, false)
		end
...
function GameModeManager:rpc_set_flow_object_set_enabled(key, enabled)
...
function GameModeManager:_hot_join_synch_object_sets(sender, player)
	for name, set in pairs(self._object_sets) do
		...
		if set.enabled then
			RPC.rpc_set_flow_object_set_enabled(sender, set.key, true)
		end
	end
end
...
function GameModeManager:_init_game_mode(game_mode_key, win_score, time_limit)
	...
	if script_data.settings.dedicated_server then
		local admin_spawning = Managers.admin:spawning(game_mode_key)
...
function GameModeManager:server_update(dt, t)
	...
	if not self._end_conditions_met and not EDITOR_LAUNCH then
		local ended, winning_team, end_of_round_only, total_victory = self._game_mode:evaluate_end_conditions()
		local win_scale = self._game_mode:win_scale()

		self._game_mode_music:update(dt, t, win_scale, self._game_mode_key, self._level_key)

		if script_data.debug_win then
			ended = true
			winning_team = Managers.state.team:team_by_name(script_data.debug_win)
			script_data.debug_win = nil
			end_of_round_only = false
		end

		if ended then
			if not end_of_round_only then
				if Managers.lobby.server then
					Managers.state.stats_collector:match_finished(self._game_mode_key, winning_team, total_victory)
				end
...
function GameModeManager:win_score()
	return self._game_mode:win_score()
end
...
function GameModeManager:override_win_score(score)
	self._game_mode:set_win_score(score)
end
...
function GameModeManager:game_mode_description_announcement(local_player)
	return self._game_mode:game_mode_description_announcement(local_player)
end

function GameModeManager:last_man_standing_announcement(local_player)
	return self._game_mode:last_man_standing_announcement(local_player)
end
...
function GameModeManager:event_player_unit_died(dead_player, attacking_player, gear_name, is_instakill, damagers, damage_type, attack_name, riposte, range, hit_zone)
	...
	if self_kill then
		...
		Managers.state.event:trigger("player_self_kill", dead_player, damage_type, is_instakill)
	else
		...
		if is_team_kill then
			...
			Managers.state.event:trigger("player_enemy_kill", dead_player, attacking_player, gear_name, is_instakill, damagers, attack_name, riposte, range, hit_zone)
		end
	end
...
function GameModeManager:event_player_knocked_down(knocked_down_player, attacking_player, gear_name, damagers, damage_type, attack_name, riposte)
	local level_event
	local game_mode_key = self:game_mode_key()
	...
	if self_knock_down then
		...
		if is_team_knock_down then
			...
			Managers.state.event:trigger("player_enemy_kd", knocked_down_player, attacking_player, gear_name, damagers, attack_name, riposte)
		end
	end
...
function GameModeManager:event_objective_being_captured(captured_unit, side)
	self:trigger_event("objective_being_captured", captured_unit, side)
end

function GameModeManager:event_objective_captured(capturing_player, captured_unit, side, previous_owner, no_previous_cap)
	self:trigger_event("objective_captured", capturing_player, captured_unit, previous_owner, no_previous_cap)
	Managers.state.stats_collector:objective_captured(capturing_player, captured_unit, self:game_mode_key())
end

function GameModeManager:event_objective_unlocked_by_time(unit, reason, owner)
	self:trigger_event("objective_unlocked_by_time", unit, reason, owner)
end
...
function GameModeManager:event_objective_captured_assist(assist_player, captured_unit)
	...
	Managers.state.stats_collector:objective_captured_assist(assist_player, captured_unit, self:game_mode_key())
end

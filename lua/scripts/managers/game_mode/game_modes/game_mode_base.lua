...
function GameModeBase:allowed_spawns(team)
	...
	elseif game then
		if not team or not team.game_object_id then
			return math.huge
		end
...
function GameModeBase:allowed_spawn_modes(player)
...
function GameModeBase:set_win_score(score)
...
function GameModeBase:game_mode_description_announcement()
	return ""
end

function GameModeBase:last_man_standing_announcement()
	return ""
end
...
function GameModeBase:hud_timer_text()
	...
	if self._time_limit then
		t = self._time_limit - math.max(Managers.time:time("round") or 0, 0)

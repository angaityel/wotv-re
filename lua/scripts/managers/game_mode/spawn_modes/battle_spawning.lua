...
function BattleSpawning:_allowed_spawns_spawn_owner(team)
	if self._spawned then
		return 0
	else
		BattleSpawning.super._allowed_spawns_spawn_owner(self, team)
	end
end
...
function BattleSpawning:update(dt)
	...
	if not self._spawned then
		self._teams_ready = self:_are_teams_ready() or self._teams_ready and t >= self._spawn_time_left
...
function BattleSpawning:_pause_round_timer(round_time)
	local new_round_time = math.min(round_time, -10)

	Managers.time:set_time("round", new_round_time)
	Managers.state.network:send_rpc_clients("rpc_synch_round_time", new_round_time, false)

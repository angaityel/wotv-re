...
function PulseSpawning:next_spawn_time(player)
	...
	if spawn_settings then
		...
		local spawn_time = spawn_settings.pulse_time

		if t < 0 then
			return 0
		elseif spawn_time - t > self._settings.minimum_pulse_time then
			return spawn_time
		else
			return spawn_time + spawn_settings.pulse_length
		end
	else
		return math.huge
	end

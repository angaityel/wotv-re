...
function PersistenceManagerServer:post_init()
	...
	self._settings.project_id = SteamGameServer.app_id()
...
function PersistenceManagerServer:event_player_joined(player)
	...
	if player.remote and profile_id ~= -1 then
		player.round_time_joined = Managers.time:time("round") or 0
...
function PersistenceManagerServer:cb_profile_attributes_received(profile_id, player, response)
	if response.error == nil then
		...
		Managers.state.event:trigger("profile_attributes_received", player)
	else
...
function PersistenceManagerServer:_save_profiles()
	...
	for profile_id, profile_data in pairs(self._profiles) do
		...
		for stat_name, stat_props in pairs(StatsContexts.player) do
			if stat_props.backend.save and not stat_props.backend.exclude_from_end_of_round then
				if stat_props.type == "compound" then
					...
					profile_stats_to_save[stat_name][profile_id] = compound_stat:get_backend_data()
				else
...
function PersistenceManagerServer:save_partial_player_progress(player)
	local xp_to_save = self._stats:get(player:network_id(), "experience_round") - self._stats:get(player:network_id(), "experience_saved")
	local coins_to_save = self._stats:get(player:network_id(), "coins_round") - self._stats:get(player:network_id(), "coins_saved")
	local save_bundle = {
		experience = {
			[Backend.PROFILE_ATTRIBUTE_INC] = xp_to_save
		},
		coins = {
			[Backend.PROFILE_ATTRIBUTE_INC] = coins_to_save
		}
	}

	Managers.backend:update_profile_attributes(player.backend_profile_id, save_bundle, callback(self, "cb_partial_progress_saved", player))
end

function PersistenceManagerServer:cb_partial_progress_saved(player, response)
	if response.error == nil then
		printf("Partial progress saved for player %q (%d)", player:name(), player.backend_profile_id)

		local player_network_id = player:network_id()
		local xp_saved = self._stats:get(player_network_id, "experience_round")
		local coins_saved = self._stats:get(player_network_id, "coins_round")

		self._stats:set(player_network_id, "experience_saved", xp_saved)
		self._stats:set(player_network_id, "coins_saved", coins_saved)
	else
		printf("Error saving partial progress for player %q (%d): %s", player:name(), player.backend_profile_id, response.error)
	end
end

function PersistenceManagerServer:save_short_term_goal(player, goal_end_time, goal_end_xp, goal, current_level)
	local save_bundle = {
		short_term_goal_end_time = {
			[Backend.PROFILE_ATTRIBUTE_SET] = tostring(goal_end_time)
		},
		short_term_goal_end_xp = {
			[Backend.PROFILE_ATTRIBUTE_SET] = tostring(goal_end_xp)
		},
		short_term_goal = {
			[Backend.PROFILE_ATTRIBUTE_SET] = tostring(goal)
		},
		current_short_term_level = {
			[Backend.PROFILE_ATTRIBUTE_SET] = tostring(current_level)
		}
	}

	Managers.backend:update_profile_attributes(player.backend_profile_id, save_bundle, callback(self, "cb_short_term_goal_saved", player))
end

function PersistenceManagerServer:cb_short_term_goal_saved(player, response)
	if response.error == nil then
		printf("Short Term Goal Saved for player %q (%d)", player:name(), player.backend_profile_id)
	else
		printf("Error saving short term goal for player %q (%d): %s", player:name(), player.backend_profile_id, response.error)
	end
end

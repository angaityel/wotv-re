...
function TelemetryCollector:event_player_enemy_kd(victim, killer, gear_name, attack_name)
	print("TelemetryCollector:event_player_enemy_kd")

	if Unit.alive(victim.player_unit) and Unit.alive(killer.player_unit) then
		print("TelemetryCollector:event_player_enemy_kd", victim, killer, gear_name, attack_name)
...
function TelemetryCollector:event_player_enemy_kill(victim, killer, gear_name, is_instakill, damagers)
	print("TelemetryCollector:event_player_enemy_kill")

	if is_instakill and Unit.alive(victim.player_unit) and Unit.alive(killer.player_unit) then
		print("event_player_enemy_kill", victim, killer, gear_name, is_instakill, damagers)
		Telemetry.write_record("kills", Application.utc_time(), self._map_id, self._game_mode, killer.team.name, Unit.world_position(killer.player_unit, 0), Gear[gear_name].hash, killer.backend_profile_id, victim.team.name, Unit.world_position(victim.player_unit, 0), victim.backend_profile_id)
	end
end

function TelemetryCollector:event_player_spawned_in_area(player, player_pos)
	print("TelemetryCollector:event_player_spawned_in_area", player, player_pos)
...
function TelemetryCollector:event_player_spawned_at_unit(player, squad_player, player_pos)
	print("TelemetryCollector:event_player_spawned_at_unit", player, squad_player, player_pos)
...
function TelemetryCollector:save_round(player)
	print("TelemetryCollector:save_round", player)
	Telemetry.write_record("rounds", Application.utc_time(), self._map_id, self._game_mode, Managers.time:time("round") - (player.round_time_joined or 0), Managers.state.stats_collection:get(player:network_id(), "experience_round"), player.backend_profile_id)
end

function TelemetryCollector:finalize()
	print("TelemetryCollector:finalize()")

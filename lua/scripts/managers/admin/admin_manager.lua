...
function AdminManager:_setup_spawning(settings)
	self._spawning_settings = settings.spawning
	self._spawning_settings_override = settings.spawning_override
end
...
function AdminManager:spawning(game_mode_key)
	return self._spawning_settings_override and self._spawning_settings_override[game_mode_key] or self._spawning_settings
end

function AdminManager:update(dt, t)
	Profiler.start("AdminManager")
	Profiler.start("auto_rules")
	self._auto_rules:update(dt, t)
	Profiler.stop()
	Profiler.start("auto_balance")
	self._auto_balancer:update(dt, t)
	Profiler.stop()
	Profiler.start("server_stats_printer")
	self._server_stats_printer:update(dt, t)
	Profiler.stop()
	Profiler.start("network_performance_rules")
	self._network_performance_rules:update(dt, t)
	Profiler.stop()
	Profiler.start("rcon_server")
	self._rcon_server:update(dt, t)
	Profiler.stop()
	Profiler.start("update_kick_list")
	self:_update_kick_list(dt, t)
	Profiler.stop()
	Profiler.start("update_rcon_admin")
	self:_update_rcon_admin(dt, t)
	Profiler.stop()
	Profiler.start("anti_cheat")

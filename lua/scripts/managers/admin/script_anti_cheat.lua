...
function ScriptAntiCheat:init(settings)
	...
	if settings.enforce and GameSettingsDevelopment.anti_cheat_enabled then
		self:_connect(self._settings.anti_cheat_dns or ANTI_CHEAT_DNS, self._settings.anti_cheat_port or ANTI_CHEAT_PORT)
...
function ScriptAntiCheat:_connect(dns, port)
	AntiCheatServer.connect(ScriptAntiCheat.game_id, dns, port, callback(self, "cb_connection_status"))
end

...
function Team:init(name, config, team_reload_context, world)
	self.name = name
	self.side = nil
	self.members = {}
	...
	for i = 1, config.number_of_squads do
		local squad_context = team_reload_context.squad_contexts and team_reload_context.squad_contexts[i] or {}

		self.squads[i] = Squad:new(i, config.max_squad_members, world, self.name, squad_context)
	end
...
function Team:reload_loading_context()
	...
	for index, squad in ipairs(self.squads) do
		local squad_context = {
			locked = squad:locked(),
			animal_index = squad:animal_index()
		}

		context.squad_contexts[index] = squad_context
	end
...
function Team:create_game_object()
	...
	local team_locked_observer_cam = self.name ~= "observer" and Managers.state.game_mode:team_locked_observer_cam(self)
...
function Team:create_squad_game_objects()
	for _, squad in ipairs(self.squads) do
		squad:create_game_object()
	end
end

function Team:squad(squad_id)
	return self.squads[squad_id]
end

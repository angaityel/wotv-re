...
function EffectHelper.play_surface_material_effects(effect_name, world, hit_unit, position, rotation, normal, armour_sound_type, player_unit)
	...
	if decal_settings then
		...
		if decal_settings.override then
			id = Unit.project_decal(hit_unit, projector_space, projector_extents, decal_settings.override)
		else
			local material_drawer_mapping = EffectHelper.create_surface_material_drawer_mapping(effect_name)

			id = Unit.project_decal(hit_unit, projector_space, projector_extents, "surface_material", material_drawer_mapping)
		end
	...
	if sound then
		...
		if armour_sound_type then
			TimpaniWorld.set_parameter(timpani_world, event_id, "character", armour_sound_type)

			local locomotion_system = ScriptUnit.extension(player_unit, "locomotion_system")
			local has_assassin_perk = locomotion_system:has_perk("backstab")
			local assassin_perk_event = has_assassin_perk and (locomotion_system.player.remote and "activated_husk" or "activated_player") or "not_activated"

			TimpaniWorld.set_parameter(timpani_world, event_id, "assasin", assassin_perk_event)
		end
	end
...
function EffectHelper.flow_cb_play_footstep_surface_material_effects(effect_name, unit, object, foot_direction)
	...
	local armour_sound_type = Unit.get_data(unit, "armour_sound_type")
	local world = Managers.world:world("level_world")
	local physics_world = World.physics_world(world)
	local hit_cb = callback(EffectHelper, "footstep_raycast_result", effect_name, foot_direction, armour_sound_type, unit)

	PhysicsWorld.make_raycast(physics_world, hit_cb, "closest", "types", "statics", "collision_filter", "ground_material_check"):cast(raycast_position + Vector3(0, 0, raycast_offset), raycast_direction, raycast_range, world)
end

function EffectHelper:footstep_raycast_result(effect_name, foot_direction, armour_sound_type, unit, hit, position, distance, normal, actor)
	if hit then
		local hit_unit = Actor.unit(actor)
		local world = Managers.world:world("level_world")
		local rotation = Quaternion.look(Vector3(0, 0, -1), foot_direction)

		EffectHelper.play_surface_material_effects(effect_name, world, hit_unit, position, rotation, normal, armour_sound_type, unit)
	end
end

function EffectHelper.query_material_surface(hit_unit, position, rotation)
	...
	return Unit.query_material(hit_unit, query_start_position, query_end_position, {
		"surface_material"
	})
end

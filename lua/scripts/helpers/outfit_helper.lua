...
function OutfitHelper._gear_mean_speed(gear_name)
	...
	elseif gear_settings.category == "lance" then
		...
		for _, attack_name in ipairs(gear_settings.menu_stats_attacks) do
			...
			sum_speed = sum_speed + (attack.charge_time + attack.charged_attack_time)
			num_attacks = num_attacks + 1
		end
	end
...
function OutfitHelper.profile_hidden(config)
	local release_setting = ReleaseSettings[config.release_name or "default"]
	...
	if config.required_dlc then
		return DLCSettings[config.required_dlc] and not DLCSettings[config.required_dlc]()
	end
...
function OutfitHelper.outfit_editor_gear_options(category)
	...
	for _, config in ipairs(sorted_table) do
		if category == config.category then
			local available, unavalible_reason = ProfileHelper:is_entity_avalible("gear", config.gear_name, "gear", config.gear_name, config.release_name, config.developer_item)
...
function OutfitHelper.outfit_editor_armour_options(category)
	...
	for _, config in ipairs(sorted_table) do
		if category == config.category then
			local available, unavalible_reason = ProfileHelper:is_entity_avalible("armour", config.armour_name, "armour", config.armour_name, config.release_name, config.release_name, config.developer_item)
...
function OutfitHelper.outfit_editor_helmet_options(category)
	...
	for _, config in ipairs(sorted_table) do
		if category == config.category then
			local available, unavalible_reason = ProfileHelper:is_entity_avalible("helmet", config.helmet_name, "helmet", config.helmet_name, config.release_name, config.developer_item)
...
function OutfitHelper.outfit_editor_basic_perk_options(category, player_profile)
	...
	print("OutfitHelper.outfit_editor_basic_perk_options( category, player_profile )   USED??")

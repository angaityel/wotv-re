...
function ItemGridMenuContainer:render(dt, t, gui, layout_settings)
	if layout_settings.rect_background_color then
		...
		Gui.rect(gui, Vector3(self._x, y, self._z), Vector2(self._items_total_width, bgr_height), color)

		if layout_settings.render_border then
			local border_c = layout_settings.border_color or {
				255,
				255,
				255,
				255
			}
			local border_color = Color(border_c[1], border_c[2], border_c[3], border_c[4])
			local rect = {
				x,
				y,
				math.floor(self._items_total_width),
				math.floor(bgr_height)
			}

			MenuHelper:render_border(gui, rect, layout_settings.border_thickness, border_color, self._z + 50)
		end
	end

...
VideoSettingsMenuPage = class(VideoSettingsMenuPage, WotvSubMenuPage)
...
function VideoSettingsMenuPage.create_from_config(compiler_data, page_config, parent_page, item_groups, callback_object)
	local config = {
		type = "video_settings",
...
function VideoSettingsMenuPage:on_enter(on_cancel)
	if not on_cancel then
		self._original_user_settings = {
			...
			apex_cloth = Application.user_setting("apex_cloth"),
			disable_apex_cloth = Application.user_setting("disable_apex_cloth"),
			apex_lod_resource_budget = Application.user_setting("apex_lod_resource_budget")
		}
		self._original_render_settings = {
...
function VideoSettingsMenuPage:cb_apply_changes()
	self:_apply_changes_made(self._changed_user_settings, self._changed_render_settings)
...
function VideoSettingsMenuPage:_apply_changes_made(user_settings, render_settings)
...
function VideoSettingsMenuPage:cb_keep_changes_popup_item_selected(args)
	...
	elseif args.action == "revert_changes" then
		self:_apply_changes_made(self._original_user_settings, self._original_user_settings)
...
function VideoSettingsMenuPage:cb_graphics_quality_options()
	...
	if saved_option == nil or saved_option == options[2].key then
		selected_index = 2
	elseif saved_option == options[1].key then
		selected_index = 1
	...
	elseif saved_option == options[5].key then
		selected_index = 5
	elseif saved_option == options[6].key then
		selected_index = 6
	end
...
function VideoSettingsMenuPage:cb_apex_cloth_options()
	local options = {
		{
			key = "off",
			value = L("menu_off")
	...
	local saved_option = T(self._changed_user_settings.apex_cloth, self._original_user_settings.apex_cloth)
	local selected_index = 1
...
function VideoSettingsMenuPage:cb_apex_cloth_option_changed(option, called_from_graphics_quality)
	local level = option.key

	self._changed_user_settings.apex_cloth = level

	local quality = ApexClothQuality[level]

	self._changed_user_settings.disable_apex_cloth = quality.disable_apex_cloth
	self._changed_user_settings.apex_lod_resource_budget = quality.apex_lod_resource_budget
...
function VideoSettingsMenuPage:cb_lod_options()
	...
	for i = 1, num_options do
		...
		local delta = math.abs(saved_option - option.key)
...
function VideoSettingsMenuPage:cb_gamma_options()
	...
	for i = 0.8, 1.6, 0.1 do
		...
		options[index] = {
			key = i,
			value = tonumber(string.format("%.1f", i))
		}

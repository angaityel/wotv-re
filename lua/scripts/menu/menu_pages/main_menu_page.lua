...
function MainMenuPage:on_enter()
	...
	self:_try_callback(self.config.callback_object, self.config.on_enter_page, self.config.on_enter_page_args and unpack(self.config.on_enter_page_args))
end
...
function MainMenuPage:render(dt, t)
	...
	self:_render_page_links(dt, t, self._gui, layout_settings.page_links)
...
function MainMenuPage:_render_page_links(dt, t, gui, layout_settings)
	if self._page_links then
		self._page_links:render(dt, t, gui, layout_settings)
	end
end

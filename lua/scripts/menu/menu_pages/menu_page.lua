	...
	if music_events and music_events[page_event] then
		for i, config in ipairs(music_events[page_event]) do
			if not config.play_once or not self._played_music_events[config.event] then
				local id = Managers.music:trigger_event(config.event)

				if config.parameter_func then
					local parameters = {
						self:_try_callback(self.config.callback_object, config.parameter_func)
					}

					for i = 1, #parameters, 2 do
						local p1, p2 = parameters[i], parameters[i + 1]

						Managers.music:set_parameter(id, p1, p2)
					end
				end
...
function MenuPage:_update_mouse_hover(input)
	...
	if not mouse_pos then
		if Window.show_cursor() or not self:current_highlight() then
	...
	for index, item in pairs(self._items) do
		if item:is_mouse_inside(mouse_pos.x, mouse_pos.y, input:get("select_down")) and item:visible() and (item:visible_in_demo() or not item:disabled() and not item.config.hidden and not item:removed()) then
...
function MenuPage:_render_button_info(layout_settings)
...
function MenuPage:cb_goto_menu_page(id)
	self._menu_logic:goto(id)
end

function MenuPage:cb_goto_root()
	local page = self

	fassert(page.config.parent_page, "Already at root page")

	repeat
		page = page.config.parent_page
	until page.config.parent_page == nil

	self._menu_logic:change_page(page, nil)
end
...
function MenuPage:current_highlight()
...
function MenuPage:move_up()
	if self.config.reverse_input_direction then
		self:traverse_item_left()
	else
		self:traverse_list_up()
	end
end

function MenuPage:move_down()
	if self.config.reverse_input_direction then
		self:traverse_item_right()
	else
		self:traverse_list_down()
	end
end
...
function MenuPage:traverse_list_up()
	local index
...
function MenuPage:traverse_list_down()
	local index
...
function MenuPage:traverse_item_left()
	if self:_highlighted_item() then
		self:_highlighted_item():on_move_left()
	end
end

function MenuPage:traverse_item_right()
	if self:_highlighted_item() then
...
function MenuPage:menu_deactivated(tab)
	for index, item in pairs(self._items) do
		if item.config.page and item.config.page == self then
			print("[MenuPage:menu_deactivated()] Item found with same page as parent page! Item name:", item.config.name, "Item type:", item.config.type, "Page type:", item.config.page.config.type)
		end

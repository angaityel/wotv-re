...
function DropDownListMenuPage:on_enter()
	...
	for _, option in pairs(options) do
		local item_config = {
			...
			on_select_args = {
				option,
				self.config.on_option_changed_args
			},
			text = option.value,
		...
		local item = rawget(_G, layout_settings.drop_down_list.item_type or "TextMenuItem").create_from_config({
			world = self._world
...
function DropDownListMenuPage:_update_mouse_hover(input)
	DropDownListMenuPage.super._update_mouse_hover(self, input)

	local highlighted_item = self:_highlighted_item()

	if not highlighted_item and input:has("select_left_click") and input:get("select_left_click") then
		self:_try_cancel()
	end
end
...
function DropDownListMenuPage.create_from_config(compiler_data, page_config, parent_page, item_groups, callback_object)
	local config = {
		...
		on_option_changed_args = page_config.on_option_changed_args or {},

...
function MainMenu:init(state, world, controller_settings, menu_settings, menu_definition, menu_callbacks, menu_data)
	...
	self._menu_shield_units = menu_data.menu_shield_units
	self._menu_main_shield_unit = menu_data.menu_main_shield_unit
...
function MainMenu:set_active(active)
	MainMenu.super.set_active(self, active)
	self:_activate_banner(active)
end

function MainMenu:_activate_banner(active)
	if active then
		local camera_dummy_unit = self._camera_dummy_units[self._current_camera]
		local cam_pos = Unit.local_position(camera_dummy_unit, 0)
		local cam_rot = Unit.local_rotation(camera_dummy_unit, 0)
		local cam_fwd = Quaternion.forward(cam_rot)

		Unit.set_local_position(self._menu_banner_unit, 0, cam_pos + cam_fwd * 1.8)
		Unit.set_local_rotation_and_scale(self._menu_banner_unit, 0, cam_rot, Vector3(0.232603, 0.232603, 0.232603))
	end

	Unit.set_unit_visibility(self._menu_banner_unit, true)
end

function MainMenu:menu_banner_unit()
...
function MainMenu:_load_camera()
	...
	camera_manager:set_variable("menu_level_viewport", "look_controller_input", Vector3Box(Vector3.zero()))
	camera_manager:change_environment("default", 0)
end

function MainMenu:_spawn_banner_unit()
	self._menu_banner_unit = World.spawn_unit(self._state.parent.level_world, "units/menu/menu_banner_2")
end
...
function MainMenu:camera_dummy_unit()
	return self._camera_dummy_units[self._current_camera]
end

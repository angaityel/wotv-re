...
function TextureButtonMenuItem:update_size(dt, t, gui, layout_settings)
	...
	self._width = layout_settings.padding_left + (layout_settings.texture_left_width or 0) + (layout_settings.texture_forced_width or self._text_width) + layout_settings.text_padding * 2 + (layout_settings.texture_right_width or 0) + layout_settings.padding_right
	self._height = (layout_settings.texture_height or self._text_height + layout_settings.text_offset_y) + layout_settings.padding_top + layout_settings.padding_bottom
end
...
function TextureButtonMenuItem:render(dt, t, gui, layout_settings)
	...
	local texture_color = texture_c and Color(texture_c[1], texture_c[2], texture_c[3], texture_c[4])
	...
	local texture_middle_x = math.floor(texture_left_x + (layout_settings.texture_left_width or 0))
...
function TextureButtonMenuItem.create_from_config(compiler_data, config, callback_object)
	...
	return TextureButtonMenuItem:new(button_config, compiler_data.world)
end

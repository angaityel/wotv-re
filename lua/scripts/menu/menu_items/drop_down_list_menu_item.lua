...
function DropDownListMenuItem:render_from_child_page(dt, t, gui, layout_settings)
	if not layout_settings.do_not_render_from_child_page then
...
function DropDownListMenuItem:render(dt, t, gui, layout_settings, rendered_from_child)
	...
	ScriptGUI.text(gui, text, font, font_size, font_material, Vector3(math.floor(self._x + layout_settings.padding_left), math.floor(self._y + layout_settings.padding_bottom), self._z + 3), Color(color[1], color[2], color[3], color[4]), shadow_color, shadow_offset)
end
...
function DropDownListMenuItem.create_from_config(compiler_data, config, callback_object)
	local item_config = {
		...
		sounds = config.parent_page.config.sounds.items.drop_down_list or MenuSettings.sounds.default.items.drop_down_list
	}

	return DropDownListMenuItem:new(item_config, compiler_data.world)
end

...
function OutfitCheckboxMenuItem:update_disabled()
	if GameSettingsDevelopment.unlock_all then
		...
		local avalible, unavalible_reason = ProfileHelper:is_entity_avalible(self.config.unlock_type, self.config.unlock_key, self.config.entity_type, self.config.entity_name, self.config.release_name, self.config.developer_item)

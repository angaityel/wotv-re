...
function VideoMenuItem:init(config, world)
	...
	self._video_loaded = Managers.package:has_loaded("resource_packages/splash_videos")

	Managers.state.event:register(self, "video_loaded", "event_video_loaded", "video_unloaded", "event_video_unloaded")
end
...
function VideoMenuItem:destroy()
	...
	event_manager:unregister("video_loaded", self)
	event_manager:unregister("video_unloaded", self)
...
function VideoMenuItem:event_video_loaded()
	self._video_loaded = true
end

function VideoMenuItem:event_video_unloaded()
	self._video_loaded = false
end

function VideoMenuItem:render(dt, t, gui, layout_settings)
	if not self._video_loaded then
		return false
	end
	...
	if self._first_frame then
		...
		local music_event = layout_settings.video and layout_settings.video.music_event or self.config.video and self.config.video.music_event or self.config.music_event or nil
	...
	if not self._last_frame and VideoPlayer.current_frame(player) == VideoPlayer.number_of_frames(player) then
...
function VideoMenuItem.create_from_config(compiler_data, config, callback_object)
	local config = {
		...
		music_event = config.music_event
	}

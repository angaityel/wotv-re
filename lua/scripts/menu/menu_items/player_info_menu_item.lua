...
function PlayerInfoMenuItem:render(dt, t, gui, layout_settings)
	...
	local play_time = player.play_time or "?"
	local play_time_text = self:_format_timer_text(math.floor(play_time))
	local min, max = Gui.text_extents(gui, play_time_text, font, layout_settings.font_size)
	local width = max[1] - min[1]

	ScriptGUI.text(gui, play_time_text, font, layout_settings.font_size, font_material, Vector3(math.floor(self._x + column_offset_x - width), math.floor(self._y + layout_settings.text_offset_y), self._z + 2), color, shadow_color, shadow_offset)
end

function PlayerInfoMenuItem:_format_timer_text(seconds)
	local t = seconds
	...
	return string.format("%01.f:%02.f:%02.f", hours, minutes, t)
end

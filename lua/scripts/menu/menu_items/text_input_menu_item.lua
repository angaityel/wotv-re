...
function TextInputMenuItem:set_text(text, ignore_index)
	self._text = text

	if not ignore_index then
...
function TextInputMenuItem:text_width()
...
function TextInputMenuItem:render(dt, t, gui, layout_settings)
	...
	self:_render_marker(gui, dt, t, text_x, text_y, layout_settings)
...
function TextInputMenuItem:_render_marker(gui, dt, t, x, y, layout_settings)
	local marker_x = x + self._marker_offset_x - layout_settings.marker_width / 2 + (layout_settings.marker_offset_x or 0)
	local marker_y = y + (layout_settings.marker_offset_y or 0)

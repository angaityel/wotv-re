...
function ObjectiveUnitDamage:add_damage(attacker_player, attacker_unit, damage_type, damage, position, normal, actor, damage_range_type)
	...
	if not self:can_receive_damage(attacker_unit) then
		return
	end
	...
	if Managers.lobby.server and attacker_player then
		...
		RPC.rpc_show_damage_number(player_network_id, player_id, NetworkLookup.damage_types[damage_type], math.floor(modified_damage), position, NetworkLookup.damage_range_types[damage_range_type], NetworkLookup.hit_zones["n/a"])
	end

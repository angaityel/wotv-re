...
function WeaponRangedProjectileBase:init(world, unit, user_unit, player, id, user_locomotion)
	WeaponRangedProjectileBase.super.init(self, world, unit, user_unit, player, id, user_locomotion)
	...
	self._maximum_ammo = settings.starting_ammo
	self._current_ammo = self._maximum_ammo

	self:_spawn_projectile_dummy(self:projectile_name())
...
function WeaponRangedProjectileBase:_spawn_projectile_dummy(projectile_name)
	...
	local projectile_dummy_unit = World.spawn_unit(self._world, projectile_settings.dummy_unit)
...
function WeaponRangedProjectileBase:_show_projectile_dummy()
	...
	Unit.flow_event(self._projectile_dummy_unit, "lua_spawn_projectile_effect")
...
function WeaponRangedProjectileBase:_hide_projectile_dummy()
	...
	Unit.flow_event(self._projectile_dummy_unit, "lua_despawn_projectile_effect")
...
function WeaponRangedProjectileBase:release_projectile(slot_name, ...)
	WeaponRangedProjectileBase.super.release_projectile(self, slot_name, ...)
...
function WeaponRangedProjectileBase:_update_ammunition(dt, t)
	if not self._husk_gear then
		...
		if ScriptUnit.has_extension(user_unit, "area_buff_system") then
			...
			local ammo_regen_rate = self._settings.ammo_regen_rate * enc_multiplier

			if self._current_ammo < self._maximum_ammo then
				self._current_ammo = math.clamp(self._current_ammo + dt * ammo_regen_rate, 0, self._maximum_ammo)
...
function WeaponRangedProjectileBase:set_sheathed(sheathed)
	if sheathed and self._projectile_dummy_unit then
		self:_hide_projectile_dummy()
	end
end
...
function WeaponRangedProjectileBase:hot_join_synch(sender, player, player_object_id, slot_name)
	if self:has_visible_projectile() then
		RPC.rpc_synch_ready_projectile(sender, player_object_id, NetworkLookup.inventory_slots[slot_name], NetworkLookup.projectiles[self:projectile_name()])
	end
end

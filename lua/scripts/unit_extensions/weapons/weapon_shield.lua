...
WeaponShield = class(WeaponShield)

function WeaponShield:init(world, unit, user_unit, player, id, user_locomotion)
	self._world = world
	self._unit = unit
...
function WeaponShield:set_sheathed(sheathed)
	return
end

function WeaponShield:start_attack(charge_factor, attack_name, abort_attack_func, attack_duration)
	self._attacking = true
	self._current_attack = {
		last_attack_time = 0,
		charge_time = charge_factor,
...
function WeaponShield:shield_bash_hit_cb(hit_unit, actor, normal, position)
	...
	elseif Unit.get_data(hit_unit, "health") then
		...
		WeaponHelper:perk_attack_hit_damagable_prop(world, user_unit, unit, hit_unit, position, normal, actor, current_attack, impact_direction)
	else
		if Unit.get_data(hit_unit, "soft_target") then
			hard_hit = false
		else
		...
		WeaponHelper:perk_attack_hit_non_damagable_prop(world, self._user_unit, unit, hit_unit, position, normal, actor, current_attack, impact_direction, self._properties)
	end
...
function WeaponShield:can_throw()
	return false
end
...
function WeaponShield:player_dead()
	return
end

WeaponShield._check_parry = WeaponMelee._check_parry
WeaponShield._check_blocking = WeaponMelee._check_blocking

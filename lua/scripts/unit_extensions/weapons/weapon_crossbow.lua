...
function WeaponCrossbow:init(world, unit, user_unit, player, id, user_locomotion)
	WeaponCrossbow.super.init(self, world, unit, user_unit, player, id, user_locomotion)
	...
	local proposed_hit_section_size = self._settings.reload_hit_section_size
...
function WeaponCrossbow:fire(projectile_position, projectile_velocity, forward_direction)
	...
	if network_manager:game() then
		...
		network_manager:send_rpc_server("rpc_spawn_projectile", player_index, user_object_id, bow_object_id, projectile_name_id, gear_name_id, projectile_position, exit_velocity, gravity_multiplier, charge_value, self:_projectile_properties_id(self._properties))
	else
		...
		Managers.state.entity:register_unit(self._world, projectile_unit, player_index, user_unit, unit, false, network_manager:game(), projectile_name, gear_name, exit_velocity, gravity_multiplier, charge_value, self:_projectile_properties_id(self._properties))
	end
end
	...
	if not self._ai_gear then
		...
		if self._hook_state == "normal" and fire_input and not self._hook_transitioning then
			...
			if self._hook_state == "hitting" then
				...
				self._modification_timer = t + 0.25 * settings.reload_hit_time
				...
				self._modification_timer = t + settings.reload_miss_time

...
function PlayerBandagingTeammate:enter(old_state, target_unit, t)
	...
	elseif not Managers.lobby.lobby then
		...
		local can_be_bandaged = damage_ext:can_be_team_bandaged()
...
function PlayerBandagingTeammate:update(dt, t)
	...
	if self:_inair_check() then
		self:_exit_on_fail()
		self:anim_event("to_inair")
		self:change_state("inair")
	end

	if self._internal.interacting and Vector3.length(Unit.world_position(self._unit, 0) - Unit.world_position(self._target_unit, 0)) > PlayerUnitMovementSettings.interaction[1].settings.bandage.break_distance then
		self:_exit_on_fail()
		self:change_state("onground")
	end
end
...
function PlayerBandagingTeammate:_duration()
	local internal = self._internal
	...
	return PlayerBandagingTeammate.super._duration(self, interaction_settings) * duration_multiplier
end

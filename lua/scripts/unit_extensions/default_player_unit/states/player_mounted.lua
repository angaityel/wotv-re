...
function PlayerMounted:update_left(dt, t, unit, look_angle)
	...
	self:anim_set_variable(Unit.animation_find_variable(unit, "horse_lean_swing_height"), swing_height)

	local lean_value = swing_height * HIGH_SWING_LEAN - 1

	self:anim_set_variable(Unit.animation_find_variable(unit, "horse_lean"), lean_value)
...
function PlayerMounted:update_middle(dt, t, unit, look_angle)
	...
	self:anim_set_variable(Unit.animation_find_variable(unit, "horse_lean"), var)
...
function PlayerMounted:update_aim_target(dt, t)
	local internal = self._internal
	local unit = self._unit
...
function PlayerMounted:_check_interact(dt, t)
	...
	if interact and not self._transition then
		...
		local target, interact_type = interaction:get_interaction_target(1)
...
function PlayerMounted:_play_dismount_animation(mount)
	...
	local left = Vector3.dot(internal.aim_vector:unbox(), mount_right) > 0
...
function PlayerMounted:_begin_couch(t, couch_slot)
	...
	local gear = inventory:gear(couch_slot)
	local settings = inventory:gear_couch_settings(couch_slot)
	local couch_time = settings.couch_time

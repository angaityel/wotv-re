...
local MOVE_VELOCITY_EPSILON = 0.05
...
function PlayerClimbing:update_movement(dt, t)
	...
	if self._exiting then
		self:_update_movement_top_enter_exit(unit, dt, t)
	...
	self:anim_set_variable(self._ladder_anim_var, ladder_anim_val)
	...
	self:_move_player_unit(unit, old_pos, ladder_pos, dt, ladder_up_dir * wanted_move_dir * speed)
	self:set_target_world_rotation(ladder_bottom_rot)
	self._rotation_component:update(dt, t)
end

function PlayerClimbing:_move_player_unit(unit, old_pos, new_pos, dt, wanted_velocity)
	...
	local delta = Vector3.dot(mover_pos - old_pos, Vector3.normalize(wanted_velocity))
	local actual_speed = delta / dt

	if actual_speed < MOVE_VELOCITY_EPSILON then
		self._internal.velocity:store(Vector3(0, 0, 0))
	else
		self._internal.velocity:store(wanted_velocity)
	end
...
function PlayerClimbing:_update_movement_top_enter_exit(unit, dt, t)
...
function PlayerClimbing:_interaction_settings()
	return PlayerUnitMovementSettings.interaction[1].settings[self._interaction_type]
end
...
function PlayerClimbing:enter(old_state, ladder_unit)
	...
	if exiting then
		...
		local wanted_velocity = Vector3(0, 0, 0)

		self:_move_player_unit(unit, Mover.position(Unit.mover(unit)), pos, World.delta_time(self._world), wanted_velocity)

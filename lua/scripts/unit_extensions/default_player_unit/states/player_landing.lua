...
function PlayerLanding:init(unit, internal, world)
	PlayerLanding.super.init(self, unit, internal, world)
...
function PlayerLanding:enter(old_state, landing_type)
	...
	elseif landing_type == "light" then
	...
	elseif landing_type == "heavy_attack" or landing_type == "light_attack" then
		self:anim_event("inair_attack_land")

		self._movement_type = "still"
		self._onground_movement_type = "idle"
		internal.land_type = "attack"

		self:_halting_landing()
	else
		fassert(false, "Unknown landing type %q.", landing_type)
	end

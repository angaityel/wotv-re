...
function HumanMounted:can_reload(slot_name, aim_input)
	...
	if inventory:can_reload(slot_name) and weapon_category ~= "crossbow" and (weapon_category == "bow" or not internal.aiming) and not internal.mounting and not internal.dismounting and not internal.ghost_mode then
		return true
	end
...
function HumanMounted:can_raise_block(t)
	...
	if block_type and (not internal.swinging or t < internal.swing_abort_time) and (block_type == "shield" or not internal.wielding) and not internal.parrying and not internal.crouching and (not internal.swing_parry_recovery_time or t > internal.swing_parry_recovery_time) and not internal.mounting and not internal.dismounting and not internal.ghost_mode and (not internal.swing_recovery_time or t > internal.swing_recovery_time or block_type ~= "shield" and block_type ~= "buckler") then
		return true, slot_name
	end

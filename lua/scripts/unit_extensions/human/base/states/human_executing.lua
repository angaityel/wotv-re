...
function HumanExecuting:update(dt, t)
	...
	self._player.state_data.interaction_progress = self._complete_time + PlayerUnitMovementSettings.interaction[1].settings.execute.duration_after_kill - t

	if t > self._complete_time and self._internal.interacting and self._interaction_confirmed then
		self:_exit_on_complete()
	elseif t > self._complete_time + PlayerUnitMovementSettings.interaction[1].settings.execute.duration_after_kill and self._interaction_confirmed then

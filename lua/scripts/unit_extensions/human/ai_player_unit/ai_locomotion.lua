...
AILocomotion.SYSTEM = "locomotion_system"
...
function AILocomotion:init(world, unit, profile, player)
	...
	self._move_multiplier_var = Unit.animation_find_variable(unit, "movement_scale")
	...
	if Managers.state.network:game() then
		self:_create_game_object(unit, profile)
	end
	...
	Unit.set_data(unit, "archetype", profile.archetype)
end
...
function AILocomotion:_create_game_object(unit, profile)
	...
	local data_table = {
		...
		game_object_created_func = NetworkLookup.game_object_functions.cb_spawn_ai_unit,
...
function AILocomotion:_init_public_variables(unit)
	...
	self.stamina = {
		...
		fail_time = -math.huge,
		recharge_timer = t,
		settings = PlayerUnitMovementSettings.stamina
	}
...
function AILocomotion:afro_touch_perk(attacking_player)
	return
end
...
function AILocomotion:_setup_inventory(player_profile)
	...
	local inventory = AIUnitInventory:new(self.world, self.unit, player, self.id)

	self._inventory = inventory

	for slot_name, profile_gear in pairs(player_profile.gear) do
		inventory:add_gear(profile_gear.name, nil, slot_name, self)

		if profile_gear.wielded then
			local main_body_state, hand_anim = inventory:set_gear_wielded(slot_name, true, true)
	...
	inventory:add_armour(player_profile.armour, player_profile.armour_attachments.patterns)
	inventory:add_head(player_profile.head, player_profile.voice, player_profile.head_attachments.beard, player_profile.head_attachments.beard_color)
...
function AILocomotion:_calculate_wanted_velocity(t, dt)
	...
	if slot_name then
		local gear = self._inventory:gear(slot_name)
...
function AILocomotion:gear_impact(impact_type, damage, impact_direction, attack_name, attacking_unit)
...
function AILocomotion:lower_block()
	self.current_state:lower_block()
end
...
function AILocomotion:is_dodging()
	return self.dodging
end

...
function AIUnitDamage:network_recieve_add_custom_damage_over_time(...)
	self:add_damage_over_time(...)
end
...
function AIUnitDamage:add_damage(attacker_player, attacker_unit, damage_type, damage, position, normal, actor, damage_range_type, gear_name, attack_name)
	local own_player = Managers.player:owner(self._unit)

	Managers.state.event:trigger("player_hit_by_damaging_source", own_player, attacker_player, attack_name)
	...
	if self._ai_profile.morale then
...
function AIUnitDamage:set_invulnerable(invulnerable)
...
function AIUnitDamage:was_instakilled()
	return true
end
...
function AIUnitDamage:is_last_stand_active()
	return false
end
...
function AIUnitDamage:is_alive()
...
function AIUnitDamage:set_max_health(health)
	self._damage = 0
	self._health = health
end

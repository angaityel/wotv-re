...
function AIOnground:block_attack(direction, attacking_units_locomotion)
	...
	self._force_lower_block = false
end

function AIOnground:lower_block()
	self._force_lower_block = true
end
...
function AIOnground:_update_weapons(dt, t)
	local internal = self._internal
...
function AIOnground:_update_tether(dt, t)
...
function AIOnground:_update_crouch(dt, t)
	local internal = self._internal
...
function AIOnground:_update_pose_attempt(dt)
	...
	local proportion_x = pose_x_val > 0 and pose_x_val / 0.8 or pose_x_val / 0.8
	local proportion_y = pose_y_val > 0 and pose_y_val / 0.8 or pose_y_val / 0.8
...
function AIOnground:anim_cb_swing_finished()
...
function AIOnground:_update_block(t, dt)
	...
	if self._force_lower_block then
		self:_lower_block()
	end
end
...
function AIOnground:can_raise_block(t, dt)
	...
	can_raise = can_raise and not internal.melee_attack and not internal.parrying and not internal.blocking
